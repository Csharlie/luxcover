...
import type { LcServicesData } from '../sections/lc-services/lc-services.schema'
import type { LcContactData } from '../sections/lc-contact/lc-contact.schema'
...
export function AppFooter({ siteData }: LayoutShellProps) {
  ...
  const serviceLinks =
    (sections.find((s) => s.type === 'lc-services')?.data as LcServicesData | undefined)
      ?.services ?? []

  const contactInfo =
    (sections.find((s) => s.type === 'lc-contact')?.data as LcContactData | undefined)
      ?.contactInfo

  const footerLinks = siteData.navigation.footer ?? []
  const infoLinks = footerLinks.filter((l) =>
    ['#about', '#gallery', '#contact', '#services'].includes(l.href)
  )
  ...
  return (
    <footer
      ...
      className="bg-obsidian-950 border-t border-obsidian-800 text-gray-400"
    >
      <div
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <span className="text-gold">Lux</span>Cover
            </p>
            ...
            <a
              href="https://www.facebook.com/profile.php?id=61578654328340"
              target="_blank"
              ...
              aria-label="LuxCover Facebook"
              className="flex items-center gap-2 text-sm text-gold hover:text-gold-light transition-colors w-fit mt-1"
            >
            ...
            >
              {contactInfo?.phone && (
                <li>
                  ...
                  >
                    <Phone className="w-4 h-4 mt-0.5 text-gold flex-shrink-0" strokeWidth={1.5} />
              ...
              {contactInfo?.email && (
                <li>
                  <a
                    ...
                    className="flex items-start gap-3 text-sm hover:text-white transition-colors"
                  >
                    <Mail className="w-4 h-4 mt-0.5 text-gold flex-shrink-0" strokeWidth={1.5} />
              ...
              {contactInfo?.address && (
                <li className="flex items-start gap-3 text-sm">
                  <MapPin className="w-4 h-4 mt-0.5 text-gold flex-shrink-0" strokeWidth={1.5} />
      ...
      <div
        ...
        className="border-t border-obsidian-800"
      >
        <div className="container mx-auto px-6 max-w-7xl py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          >
            <p data-ui-id="footer-copyright" data-ui-role="footer-copyright">
              &copy; {new Date().getFullYear()} LuxCover. Minden jog fenntartva.
            </p>
